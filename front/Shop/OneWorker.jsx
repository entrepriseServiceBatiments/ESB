      ...
      <View style={styles.overlay}>
        <View style={styles.container}>
          ...
          <Text style={styles.workerTitle}>
             {worker.userName} {worker.status && <Icon name="check-circle" size={20} color="green" />}
          </Text>
          <Text style={styles.jobTitle}>{worker.jobTitle}</Text>
          <Text style={styles.workerDescription}>Hourly Rate: ${worker.hourlyRate.toFixed(2)}</Text>
          ...
          <MapView style={styles.map}> 
            <Marker
                    coordinate={{ latitude: Number(worker.latitude), longitude: Number(worker.longitude) }}
                  />
                  </MapView>
